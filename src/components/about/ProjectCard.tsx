...
export default function ProjectCard({
  ...
  return (
    <Card className="border-l-4 border-l-blue-500">
      <CardHeader className="flex justify-between p-4 bg-blue-50 dark:bg-blue-900/20">
        <div className="flex items-center gap-2">
          <LuBriefcase className="h-8 w-8 text-blue-500" />
        ...
        {projectLink && (
          <Button as={Link} color="primary" showAnchorIcon href={projectLink}>
            项目链接
          </Button>
        )}
      </CardHeader>
      <CardBody className="p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {projectChip.map((chip, index) => (
            <Chip key={`${chip}-${index}`} color="primary" variant="flat">
              {chip}
            </Chip>
          ))}
        </div>
